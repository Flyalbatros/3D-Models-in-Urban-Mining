...
def height_deviations_vertical(coord_list, min_dist, max_dist, surf, normal, inFile, indexes):
    ...
    for pt_idx in range(0,len(coord_list)):
        ...
        #print(shortest_vector_length)
        curr_normal = normal*shortest_vector_length
...
def shortest_distance(plane_pt, normal_v, query_pt):
    ...
    return np.dot(plane_query_v,normal_v)
...
def grow_regions(deviations, tree, coord_list, tolerance, normal_v, inFile, indexes):
    print("num deviations", len(deviations))
    SeedInd = deviations[::10] #select every 10 random points from the original file
    ...
    while(True):
        ...
        while len(stack)>0:
            ...
            #print("popped point")
            exp_pt = coord_list[exp_pt_ind]
            #now let's find neighbors
            dist, neighb = tree.query(exp_pt, 10)
            neighb = np.delete(neighb, np.where(dist == np.inf))
            ...
            for n_pt_ind in neighb:
                if n_pt_ind in deviations_dict:
                    ...
                    inFile.user_data[indexes[n_pt_ind]] = 10 + reg_idx + 1
                    #print(n_pt_ind)
                    pass
                else:
                    n_pt = coord_list[n_pt_ind]
                    dist, plane_neighb = tree.query(n_pt, 10)
                    plane_neighb = np.delete(plane_neighb, np.where(dist == np.inf))
                    #print(plane_neighb)
                    plane_pts = []
                    for plane_ind in plane_neighb:
                        plane_pts.append(coord_list[plane_ind])
                    loc_normal, curvature = fit_plane(plane_pts)
                    # # coord_list = list(zip(inFile.X, inFile.Y))
                    # for index in range(0, len(inFile.X)):
                    #     dist, neighbors = tree.query([inFile.x[index], inFile.y[index], inFile.z[index]], 10)
                    #     neighb_coord = []
                    ...
                    #     else:
                    #         inFile.user_data[index] = 0
                    if np.dot(loc_normal, normal_v)>tolerance:
                        #curr_region[n_pt_ind] = -1
                        #print("rejected point after plane check")
                        inFile.user_data[indexes[n_pt_ind]] = 2
                        pass
                    else:
                        ...
                        stack.append(n_pt_ind)
                        add_pts_cnt += 1
                        inFile.user_data[indexes[n_pt_ind]] = 100+reg_idx+1
...
def extract_border_pts(coord_list, region_dict, inFile, indexes):
    border_output = []
        ...
        if len(region)>10:
            ...
            for pt_ind in region:
                if pt_ind not in non_boundary_pts:
                    ...
                    #print(pt_ind)
                    inFile.user_data[indexes[pt_ind]] = 3
                else:
...
def border_pts_to_geometry_mst(coord_list,border_pt_groups, inFile, indexes):
    #prepare outfile
    try:
    ...
    for region_border in border_pt_groups:
        if len(region_border)>4:
            ...
            for border_pt_ind in region_border:
                border_pt_coord = coord_list[border_pt_ind][0:2]
            ...
            for orig in range(0,len(region_border)):
                for dest in range(0,len(region_border)):
                    if mst[orig,dest] >0:
                        ...
                        out_linestr+=linestr
                        inFile.user_data[indexes[region_border[orig]]] = 4
                        inFile.user_data[indexes[region_border[dest]]] = 4
                        print("border pt registered")
...
def regions_to_geometry_alpha(coord_list, region_dict):
    ...
    for region in region_dict:
        if len(region)>3:
            ...
            for pt_ind in region:
                pt_coord = coord_list[pt_ind][0:2]
                region_coords.append(Point(pt_coord))
            #print(region_coords)
            concave_hull, edge_points = alpha_shape(region_coords, alpha=0.1)
            print(concave_hull)
            outfile = open("wkt_test_subsurf.txt", 'a')
            outfile.write(str(concave_hull))
            outfile.write("\n")
            outfile.close()
...
def regions_to_geometry_voronoi(coord_list, region_dict, bound_geometry, distance_list, outfile, building_id, roof_surf_id):
    ...
    #prepare outfile
    # try:
    #     outfile = open("wkt_test_subsurf.txt", 'x')
    #     #outfile.close()
    # except:
    #     pass
    #     #outfile = open("wkt_test_subsurf.txt", 'a')
...
def get_normals_and_merge(roof_surf_wkts):
    ...
    for idx in range(0,len(list_normals)-1):
        ...
        for other_idx in range(idx+1,len(list_normals)):
            ...
            dot_product = 0.1
            if dot_product>0.999847:
                intersec_pt_list = []
                for pt in list(roof_surf_geoms[idx].exterior.coords) + erased_pts:
                    if pt in roof_surf_geoms[other_idx].exterior.coords:
                        if pt not in intersec_pt_list:
                            intersec_pt_list.append(pt)
                if len(intersec_pt_list)>=2:
                    old_pts = list(roof_surf_geoms[idx].exterior.coords) + list(roof_surf_geoms[other_idx].exterior.coords)
                    roof_surf_geoms[idx] = merge_geoms([roof_surf_geoms[idx], roof_surf_geoms[other_idx]])
                    for pt in old_pts:
                        if pt not in roof_surf_geoms[idx].exterior.coords:
                            erased_pts.append(pt)
                    if type(roof_surf_wkts[idx]) == type([]):
                        roof_surf_geoms[idx].append(roof_surf_geoms[other_idx])
                        idx_to_merge.append(idx)
                    else:
                        roof_surf_geoms[idx] = [roof_surf_geoms[idx],roof_surf_geoms[other_idx]]
                        #print("added")
                        idx_to_merge.append(idx)
                        #print("added2")
                    roof_surf_geoms[other_idx] = None
    ...
    #     roof_surf_geoms[idx] = merge_geoms(roof_surf_geoms[idx])
    roof_surf_geoms = [x for x in roof_surf_geoms if x is not None]
...
def point_in_polygon(roof_surf_geom, ptcloud, normal_vectors):
    if len(ptcloud.x)==0:
        return []
    points_poly_match = []
    for poly_idx in range(0, len(roof_surf_geom)):
        print("starting polygon", poly_idx)
        curr_polygon_list = []
...
# #map_curvature(inFile, tree, 5)

def workflow(start_idx, end_idx, bboxs, ids, cityGML_data, inFile, q):
    for index in range(start_idx,end_idx):
        q.put(["starting building", ids[index], index])
        reduced_pc = crop_pc(inFile,bboxs[index], ids[index])
        if reduced_pc != None:
            roof_surf_list, roof_surf_wkts = cityGML_data.get_roof_geom(ids[index])
            ...
            print("split pointcloud in parts")
            for idx in range(0,len(roof_surf)):
                if len(point_in_poly_list[idx])>2:
                    ...
                    local_dev_seeds = height_deviations_vertical(local_coord_list, 0.1, 20, roof_surf[idx].exterior.coords, n_vecs[idx], reduced_pc, point_in_poly_list[idx])
    ...
    q.put("finished buildings", start_idx, "until", end_idx)
    return
...
if __name__=='__main__':
    ...
    ids_to_valid = ["ID_0599100000634332"]
    print(ids_to_valid)
    ...
    for id_to_valid in ids_to_valid:
        ...
        if reduced_pc != None:
            ...
            #for geom in roof_surf:
                #print(str(geom)+'/n')
            point_in_poly_list = point_in_polygon(roof_surf, reduced_pc, n_vecs)
            # for loc_idx in range(0,len(point_in_poly_list)):
            #     print(len(point_in_poly_list[loc_idx]))
            #     print(roof_surf[loc_idx])
            #print("split pointcloud in parts")
