...
def comp_normals(inFile, tree, tolerance, knn_setting):
    #coord_list = list(zip(inFile.X, inFile.Y))
    for index in range(0,len(inFile.X)):
        dist, neighbors = tree.query([inFile.x[index], inFile.y[index],inFile.z[index]], knn_setting)
        neighb_coord = []
        ...
        v_normal = [0,0,1]
        if np.dot(v_normal, normal)>tolerance:
        #abs(normal[2])>(tolerance*abs(normal[1])) and abs(normal[2])>(tolerance*abs(normal[0])):
...
t0 = time.time()
inFile, tree = data_h_filter('waalhaven zuid/waalhaven_zuid_bldg - Copy15.las')
#inFile, tree = data_loader('globalPC/AHN_buildings.las')
#deviations = height_deviations(inFile, 0.2)
#print(deviations)
comp_normals(inFile, tree, .9962, 15)
